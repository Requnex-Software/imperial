...
function renderCountries(filter = '') {
  ...
  if (filtered.length === 0) {
    countryList.innerHTML = '<div class="prefix-item" style="pointer-events: none; color: #666;">No results</div>';
    return;
  }
  ...
  filtered.forEach(country => {
    ...
    if (country.code === prefixVal.textContent && country.name === 'United States') { // Simple active check
...
if (track && prevBtn && nextBtn) {
  nextBtn.addEventListener('click', () => {
    currentIndex++;
    if (currentIndex > cards.length - 3) { // Show 3 at a time roughly
       currentIndex = 0;
    }
    updateCarousel();
  });
...
function updateCarousel() {
  ...
  cards.forEach((card, index) => {
    ...
    if (index === currentIndex + 1) { // Center card active logic simplified
